...
$(function() {
  ...
  $(document).on('click', '.item', function() {
    ...
    var item_image = '<figure class="media-left"><p class="image is-128x128">'
    var button = '<div class="buttons"><button class="button is-success item-select">このアイテムを選択</button><button class="button back-to-page">検索結果に戻る</button></p></div>'
    $("#item_list").hide();
    $("#item_detail").append(article + item_image + `<img src="${item.image}"></p></figure>` + `<div class="media-content"><div class="content">` + `<p class="has-text-grey-lighter">${item.maker}<br>` + `<strong>${item.name}</strong>` + `<br>¥${item.price.toLocaleString()}（参考価格）</p>` +　`<br>` + button + `</article>` + `<span class="detail-item-data" data-detail-item='{ "amazon_url":"${item.amazon_url}", "image":"${item.image}", "item_code":"${item.item_code}", "name":"${item.name}", "price":"${item.price}", "rakuten_url":"${item.rakuten_url}", "maker":"${item.maker}" }'></span>`);
    $(item).remove();
  });
  ...
  $(document).on('click', '.item-select', function() {
    ...
    if (item1 == "") {
      $("#post_items1").val(`{ item_code: "${select_item.item_code}", name: "${select_item.name}", maker: "${select_item.maker}", price: "${select_item.price}", image: "${select_item.image}", rakuten_url: "${select_item.rakuten_url}", amazon_url: "${select_item.amazon_url}" }`);
      $("#item-results").append('<div class="column is-half-mobile is-one-third-desktop" id="item-id-1"><div class="card"><button class="delete" type="button" id="delete_button1"></button>' + card_image + `<img src="${select_item.image}"></figure></div>` + card_content + `<p>${select_item.name.substr(0, 20)}...</p>` + `<p>¥${select_item.price}</p></div></div>`);
    } else if (item1 != "" && item2 == "") {
